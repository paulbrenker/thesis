...
\begin{enumerate}
  \item \textit{Level 0 The Swamp of Pox:} Das Hypertext Transfer Protokoll (HTTP) wird als Tunnelmechanismus für Methodenaufrufe auf dem Server genutzt (Remote Procedure Call - RPC). Mechanismen des Internets werden nicht berücksichtigt.
  \item \textit{Level 1 Resources:} Anstelle von Methodenaufrufen an einen einzelnen Endpunkt ist das Design Ressourcenorientiert. 
  \item \textit{Level 2 HTTP Verbs:} Ressourceninteraktionen (Create, Read, Update und Delete) werden auf die Http Methoden GET, POST, PUT, DELETE abgebildet.
  \item \textit{Level 3 Hypermedia Controls:} Folgeoperationen sind in Form von Hyperlinks in API Responses eingebettet.(HATEOAS - Hypermedia as the engine of application state).
\end{enumerate}


Massé stellt im Buch \textit{REST API Design Rulebook} \parencite{masse_rest_2011} 82 Regeln auf, die für ein Design von REST Prinzipien konformen APIs eingehalten werden sollen. Diese Regeln sind in fünf Kategorien unterteilt:
\begin{enumerate}
...
\end{enumerate}
Das Ziel dieses Regelwerkes ist REST APIs konsistent zu designen und offene Fragen zu den REST Prinzipien zu beantworten. Auch weitere wissenschaftliche Arbeiten beschäftigen sich mit dem Erstellen von Regeln für das Design von REST APIs.

Regeln für konsistentes Design von REST APIs werden auch in der Industrie erstellt. Einige große Firmen stellen Ihre Regeln öffentlich zur Verfügung. % HERE FOOTNOTE

% Hier die verschiedenen ersteller nur erwähnen und auf deren Kontributionen eingehen. Nicht aber die Tragweite der Kontributionen erläutern. Danach dann vom REST Ruler Paper von Kotstein und Bogner direkt überleiten zum Linting und Theorie und Praxis hinter dem Linting.
